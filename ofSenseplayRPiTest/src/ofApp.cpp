...
void ofApp::setup(){
    ...
    if ( ofxMidiIn::getNumPorts() && ofxMidiOut::getNumPorts() ) {
        ...
        int portIndex = 0;
        string line_str = "Connecting to Midi In Port " + ofxMidiIn::getPortList().at(1);
        statusOutputStrs.push_back(line_str);
        myICubeX.connectMidiIn(1);

        line_str = "Connecting to Midi Out Port " + ofxMidiOut::getPortList().at(1);
        statusOutputStrs.push_back(line_str);
        myICubeX.connectMidiOut(1);
